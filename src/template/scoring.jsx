...
const Scoring = () => {
  ...
  const getDynamicFeedback = ({ Clarity, Confidence, Authenticity, emotional }) => {
    // Note: Renamed 'Emotional' to 'emotional' to match state variable
    const scores = [
  ...
  useEffect(() => {
    const fetchScore = async (vId) => {
      try {
        const response = await apiClient.get(`https://app.wezume.in/api/totalscore/${vId}`);
        setClarity(response.data.clarityScore || 0);
        setCofidence(response.data.confidenceScore || 0);
        setAuthenticity(response.data.authenticityScore || 0);
        setEmotional(response.data.emotionalScore || 0);
    ...
    const fetchUserDetails = async (uId) => {
      try {
        const response = await axios.get(`${env.baseURL}/api/videos/getOwnerByUserId/${uId}`);
        if (response.data) {
    ...
    const fetchProfilePic = async (uId) => {
      try {
        const response = await axios.get(`${env.baseURL}/users/user/${uId}/profilepic`, { responseType: 'arraybuffer' });
        if (response.data) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <AnimatedCard delay={200}>
          <View style={styles.totalScoreHeader}>
            <MaterialIcons name="emoji-events" size={30} color="#FFD700" />
            <Text style={styles.sectionTitle}>Scores</Text>
          </View>
          <Text style={styles.totalScoreText}>{(score || 0).toFixed(1)}/10</Text>
          <AnimatedScoreBar label="Clarity" score={Clarity} delay={300} />
          <AnimatedScoreBar label="Confidence" score={Confidence} delay={400} />
          <AnimatedScoreBar label="Authenticity" score={Authenticity} delay={500} />
          <AnimatedScoreBar label="Emotional Intelligence" score={emotional} delay={600} />
        </AnimatedCard>
        ...
        <AnimatedCard delay={400}>
          ...
          <View style={styles.hashtagContainer}>
            <Text style={styles.hashtag}>{getHashtags({ Clarity })}</Text>
            <Text style={styles.hashtag}>{getHashtags1({ Confidence })}</Text>
            <Text style={styles.hashtag}>{getHashtags2({ Authenticity })}</Text>
            <Text style={styles.hashtag}>{getHashtags3({ emotional })}</Text>
          </View>
        </AnimatedCard>
      </ScrollView>
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: {
    fontSize: 26, // Changed from 18
    fontWeight: 'bold',
    color: '#fff',
    marginLeft: 10,
  },
  totalScoreText: {
    fontSize: 52, // Changed from 48
    fontWeight: 'bold',
    color: '#fff',
    textAlign: 'center',
    marginVertical: 10,
  ...
  scoreBarLabel: {
    fontSize: 16,
    fontWeight: '900', // Changed from 14
  ...
  feedbackText: {
    ...
    fontWeight: '600',
    color: 'rgba(255, 255, 255, 0.9)',
    lineHeight: 22, // Increased for better spacing
  },
  hashtagContainer: {
    flexDirection: 'row',
    flexWrap: 'wrap',
