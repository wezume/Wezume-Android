...
const EMOJI = {
  ...
  HEART: '\u2764\ufe0f',            // ❤️ Red Heart (Like)
  BROKEN_HEART: '\u{1F494}',        // 💔 Broken Heart (Dislike Animation)
  PLAY: '\u25b6\ufe0f',             // ▶️ Play Button
  PAUSE: '\u23f8\ufe0f',            // ⏸️ Pause Button
  SHARE: '\u27a1\ufe0f',            // ➡️ Right Arrow (Used for share)
  PHONE: '\u{1F4DE}',               // 📞 Telephone Receiver
  EMAIL: '\u{2709}\ufe0f',          // ✉️ Envelope (Email)
  SCORE: '\u{1F3C1}',               // 🏁 Checkered Flag (Used for Score)
};
// ----------------------------------------
...
const VideoPlayer = memo(({ item, isActive, onLike, isLiked }) => {
  ...
  const handleLikePress = () => {
      ...
      if (!currentlyLiked) {
        ...
        likeHeartOpacity.value = withTiming(1, { duration: 100 });
        likeHeartScale.value = withSpring(1.2, { damping: 10, stiffness: 100 }, () => {
            likeHeartScale.value = withSpring(0.5);
            likeHeartOpacity.value = withTiming(0, { duration: 200 });
        });
      } else {
        // Animate Broken Heart on Dislike
        dislikeHeartOpacity.value = withTiming(1, { duration: 100 });
        dislikeHeartScale.value = withSpring(1.2, { damping: 10, stiffness: 100 }, () => {
            dislikeHeartScale.value = withSpring(0.5);
            dislikeHeartOpacity.value = withTiming(0, { duration: 200 });
        });
      }
    ...
    <GestureDetector gesture={doubleTapGesture}>
      <View style={styles.page}>
        ...
        <Animated.View style={[styles.playPauseOverlay, playPauseStyle]}>
          ...
          <Emoji 
            unicode={isPaused ? EMOJI.PLAY : EMOJI.PAUSE} 
            size={80} 
            color="rgba(255, 255, 255, 0.7)" 
          />
        </Animated.View>
        <Animated.View style={[styles.heartAnimationContainer, animatedLikeHeartStyle]}>
          {/* Replaced HeartIcon with Emoji */}
          <Emoji unicode={EMOJI.HEART} size={100} color="white" />
        </Animated.View>
        <Animated.View style={[styles.heartAnimationContainer, animatedDislikeHeartStyle]}>
          {/* Replaced BrokenHeartIcon with Emoji */}
          <Emoji unicode={EMOJI.BROKEN_HEART} size={100} color="white" />
        </Animated.View>
        ...
        <LinearGradient colors={['rgba(0,0,0,0.6)', 'transparent', 'rgba(0,0,0,0.6)']} style={styles.overlay}>
            <View style={styles.topControls}>
              <AnimatedIconButton onPress={() => navigation.goBack()}>
                {/* Replaced Ant (arrowleft) with Emoji */}
                <Emoji unicode={EMOJI.ARROW_LEFT} size={24} color={'#fff'} />
              </AnimatedIconButton>
            </View>
            <View style={styles.bottomControls}>
              ...
              <View style={styles.rightColumn}>
                <AnimatedIconButton onPress={handleLikePress}>
                  {/* Replaced Like (heart) with Emoji */}
                  <Emoji 
                    unicode={EMOJI.HEART} 
                    size={30} 
                    color={isLiked ? '#FF005E' : '#fff'} 
                  />
                  <Text style={styles.iconText}>{likeCount}</Text>
                </AnimatedIconButton>
                <AnimatedIconButton onPress={() => navigation.navigate('ScoringScreen', { videoId: id, userId: videoOwnerId })}>
                  {/* Replaced Score (speedometer) with Emoji */}
                  <Emoji unicode={EMOJI.SCORE} size={30} color={'#fff'} />
                  <Text style={styles.iconText}>{totalScore}</Text>
                </AnimatedIconButton>
                <AnimatedIconButton onPress={handleShare}>
                  {/* Replaced Shares (share) with Emoji */}
                  <Emoji unicode={EMOJI.SHARE} size={30} color={'#fff'} />
                </AnimatedIconButton>
                 <AnimatedIconButton onPress={handleCall}>
                  {/* Replaced Phone (phone-volume) with Emoji */}
                  <Emoji unicode={EMOJI.PHONE} size={22} color={'#fff'} />
                </AnimatedIconButton>
                <AnimatedIconButton onPress={handleEmail}>
                  {/* Replaced Whatsapp (email) with Emoji */}
                  <Emoji unicode={EMOJI.EMAIL} size={27} color={'#fff'} />
                </AnimatedIconButton>
              </View>
            </View>
          </LinearGradient>
