...
const HomeScreen = () => {
  ...
  useEffect(() => {
    const backAction = () => {
      // Only handle back press if the screen is focused
      if (isFocused) {
        Alert.alert('Hold on!', 'Are you sure you want to go back?', [
          {
            text: 'Cancel',
            onPress: () => null, // Does nothing
            style: 'cancel',
          },
          // Corrected button configuration
          { text: 'YES', onPress: () => navigation.goBack() },
        ]);
        // Return true to prevent the default back button action
        return true;
      }
      // Return false to allow the default action on other screens
      return false;
    };
