...
// Removed: import Ant from 'react-native-vector-icons/AntDesign';
// Removed: import Shares from 'react-native-vector-icons/Entypo';
// Removed: import Like from 'react-native-vector-icons/Foundation';
import Share from 'react-native-share'; // Import the share module
// Removed: import Phone from 'react-native-vector-icons/FontAwesome6';
// Removed: import Score from 'react-native-vector-icons/MaterialCommunityIcons';
// Removed: import Whatsapp from 'react-native-vector-icons/Entypo';
import RNFS from 'react-native-fs';
import env from './env';
import AsyncStorage from '@react-native-async-storage/async-storage';
const windowHeight = Dimensions.get('screen').height;

// --- Unicode Emoji/Symbol Constants ---
const BACK_ICON = '\u2B05'; // Leftwards Black Arrow
const SHARE_ICON = '\u21AA'; // Rightwards Arrow with Hook (Share symbol)
const HEART_ICON = '\u2764\uFE0F'; // Heavy Black Heart (for like)
const EMAIL_ICON = '\u2709\uFE0F'; // Envelope (for email/message)
const SCORE_ICON = '\u23F1'; // Stopwatch (Used for score/speedometer)
const PHONE_ICON = '\u260E\uFE0F'; // Black Telephone Receiver (for phone call)
// --- End Unicode Emoji/Symbol Constants ---
...
const MySwipe = () => {
  ...
  useEffect(() => {
    const fetchVideos = async (page = 0, size = 2) => {
      try {
        setLoading(true);
              ...
              <View style={styles.fullScreen}>
                ...
                <View style={styles.buttoncls}>
                  <TouchableOpacity
                    ...
                    {/* Replaced Ant icon with Unicode symbol */}
                    <Text style={[styles.iconText, {fontSize: 24, color: '#ccc'}]}>
                      {BACK_ICON}
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                <View style={styles.buttonshare}>
                  <TouchableOpacity onPress={shareOption}>
                    {/* Replaced Shares icon with Unicode symbol */}
                    <Text style={[styles.iconText, {fontSize: 30, color: '#ffffff'}]}>
                      {SHARE_ICON}
                    </Text>
                  </TouchableOpacity>
                </View>
                  ...
                  <>
                    <View style={styles.buttonmsg}>
                      <TouchableOpacity onPress={() => sendEmail(item)}>
                        ...
                        <Text style={[styles.iconText, {fontSize: 27, color: '#ffffff'}]}>
                          {EMAIL_ICON}
                        </Text>
                      </TouchableOpacity>
                    </View>
                    <View style={styles.buttonscore}>
                      <TouchableOpacity
                        ...
                        {/* Replaced Score/MaterialCommunityIcons speedometer icon with Unicode symbol */}
                        <Text style={[styles.iconText, {fontSize: 30, color: '#ffffff'}]}>
                          {SCORE_ICON}
                        </Text>
                    ...
                    <View style={styles.buttonphone}>
                      <TouchableOpacity onPress={() => makeCall(item)}>
                        ...
                        <Text style={[styles.iconText, {fontSize: 22, color: '#ffffff'}]}>
                          {PHONE_ICON}
                        </Text>
                      </TouchableOpacity>
                    </View>
...
const styles = StyleSheet.create({
  ...
  // New style for unified text icon styling
  iconText: {
    // Add common text styles if needed, like fontWeight
    fontWeight: '900',
  },
  buttoncls: {
    color: '#ffffff',
    ...
    top: 15,
    right: '89%',
    fontSize: 24, // Note: The size will be set inline for flexibility
    fontWeight: '900',
  },
  buttonscore: {
    position: 'absolute',
    ...
    right: 27,
    color: '#ffffff',
    fontSize: 30, // Note: The size will be set inline for flexibility
    zIndex: 10,
    elevation: 10,
    padding: 10,
  },
  ...
  buttonphone: {
    ...
    right: 30,
    color: '#ffffff',
    fontSize: 22, // Note: The size will be set inline for flexibility
    zIndex: 10,
    padding: 10,
    elevation: 10,
  },
